...
def test_history_detail_update_and_rerun_endpoints_work_end_to_end(tmp_path) -> None:
    ...
    create_response = client.post("/api/search", json=create_payload)
    ...
    update_response = client.put(f"/api/history/{history_id}", json=update_payload)
...
def test_update_history_endpoint_returns_404_for_missing_record(tmp_path, monkeypatch) -> None:
    ...
    response = client.put("/api/history/999", json=update_payload)
